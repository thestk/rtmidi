...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(srcdir)/rtmidi-config.in $(srcdir)/rtmidi.pc.in \
	$(top_srcdir)/doc/doxygen/Doxyfile \
	$(top_srcdir)/doc/doxygen/footer.html \
	$(top_srcdir)/doc/doxygen/header.html \
	$(top_srcdir)/doc/doxygen/tutorial.txt \
	$(top_srcdir)/doc/images/ccrma.gif \
	$(top_srcdir)/doc/images/mcgill.gif \
	$(top_srcdir)/config/depcomp $(rtmidi_inc_HEADERS) \
...
CONFIG_CLEAN_FILES = rtmidi-config rtmidi.pc
CONFIG_CLEAN_VPATH_FILES = doc/doxygen/Doxyfile \
	doc/doxygen/footer.html doc/doxygen/header.html \
	doc/doxygen/tutorial.txt doc/images/ccrma.gif \
...
am__installdirs = "$(DESTDIR)$(libdir)" \
	"$(DESTDIR)$(pkgconfigdatadir)" "$(DESTDIR)$(rtmidi_incdir)"
LTLIBRARIES = $(lib_LTLIBRARIES)
librtmidi_la_LIBADD =
am_librtmidi_la_OBJECTS = RtMidi.lo rtmidi_c.lo
librtmidi_la_OBJECTS = $(am_librtmidi_la_OBJECTS)
AM_V_lt = $(am__v_lt_$(V))
am__v_lt_ = $(am__v_lt_$(AM_DEFAULT_VERBOSITY))
am__v_lt_0 = --silent
am__v_lt_1 = 
librtmidi_la_LINK = $(LIBTOOL) $(AM_V_lt) --tag=CXX $(AM_LIBTOOLFLAGS) \
	$(LIBTOOLFLAGS) --mode=link $(CXXLD) $(AM_CXXFLAGS) \
	$(CXXFLAGS) $(librtmidi_la_LDFLAGS) $(LDFLAGS) -o $@
AM_V_P = $(am__v_P_$(V))
...
am__v_CXXLD_1 = 
SOURCES = $(librtmidi_la_SOURCES)
DIST_SOURCES = $(librtmidi_la_SOURCES)
...
STRIP = strip
VERSION = 2.1.1
abs_builddir = /Users/gary/programming/rtmidi
abs_srcdir = /Users/gary/programming/rtmidi
...
lib_LTLIBRARIES = librtmidi.la
librtmidi_la_LDFLAGS = -no-undefined -export-dynamic -version-info 3:0:0
librtmidi_la_SOURCES = \
  RtMidi.cpp \
  rtmidi_c.cpp
...
$(am__aclocal_m4_deps):
rtmidi-config: $(top_builddir)/config.status $(srcdir)/rtmidi-config.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
rtmidi.pc: $(top_builddir)/config.status $(srcdir)/rtmidi.pc.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
librtmidi.la: $(librtmidi_la_OBJECTS) $(librtmidi_la_DEPENDENCIES) $(EXTRA_librtmidi_la_DEPENDENCIES) 
	$(AM_V_CXXLD)$(librtmidi_la_LINK) -rpath $(libdir) $(librtmidi_la_OBJECTS) $(librtmidi_la_LIBADD) $(LIBS)
...
include ./$(DEPDIR)/RtMidi.Plo
include ./$(DEPDIR)/rtmidi_c.Plo
...
install-rtmidi_incHEADERS: $(rtmidi_inc_HEADERS)
	@$(NORMAL_INSTALL)
	@list='$(rtmidi_inc_HEADERS)'; test -n "$(rtmidi_incdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(rtmidi_incdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(rtmidi_incdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_HEADER) $$files '$(DESTDIR)$(rtmidi_incdir)'"; \
	  $(INSTALL_HEADER) $$files "$(DESTDIR)$(rtmidi_incdir)" || exit $$?; \
	done

uninstall-rtmidi_incHEADERS:
	@$(NORMAL_UNINSTALL)
	@list='$(rtmidi_inc_HEADERS)'; test -n "$(rtmidi_incdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(rtmidi_incdir)'; $(am__uninstall_files_from_dir)
...
installdirs-am:
	for dir in "$(DESTDIR)$(libdir)" "$(DESTDIR)$(pkgconfigdatadir)" "$(DESTDIR)$(rtmidi_incdir)"; do \
...
install-data-am: install-pkgconfigdataDATA install-rtmidi_incHEADERS
...
.PHONY: $(am__recursive_targets) CTAGS GTAGS TAGS all all-am \
	...
	install-rtmidi_incHEADERS install-strip installcheck \
	installcheck-am installdirs installdirs-am maintainer-clean \
	maintainer-clean-generic mostlyclean mostlyclean-compile \
	mostlyclean-generic mostlyclean-libtool pdf pdf-am ps ps-am \
	tags tags-am uninstall uninstall-am uninstall-libLTLIBRARIES \
	uninstall-pkgconfigdataDATA uninstall-rtmidi_incHEADERS
